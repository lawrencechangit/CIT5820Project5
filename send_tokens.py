...
def connect_to_algo(connection_type=''):
    ...
    if connection_type == "indexer":
        ...
        client = indexer.IndexerClient(
            indexer_token=algod_token, indexer_address=algod_address
        )
    else:
...
def send_tokens_algo(acl, sender_sk, txes):
    ...
    for i, tx in enumerate(txes):
        ...
        amt=tx['buy_amount']
        #unsigned_tx = acl.transaction.Transaction(sender_sk, params, receiver,amt)
        unsigned_tx=acl.transaction.PaymentTxn(sender=sender_pk,sp=params,receiver=receiver,amt=amt)
